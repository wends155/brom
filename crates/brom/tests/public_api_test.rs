...
#[derive(Debug, Clone, Serialize, Deserialize, BromEntity)]
...
async fn test_auth_policy_api_key() {
    use brom_auth::{ApiKeyStore, Permission};
    let state = common::test_app_state();
    setup_mock_tables(&state);
    ...
    let (raw_key, _) = pool.create(user_id, "test_key", Permission::Read).unwrap();
...
#[tokio::test]
async fn test_auth_policy_admin_only_access() {
    let state = common::test_app_state();
    setup_mock_tables(&state);

    // This router contains the admin CRUD handlers generated by the macro
    let app = AdminOnlyData::admin_router().with_state(state.clone());

    // Try to access the admin endpoint (GET /admin/api/entities/adminonlydata/1) without any authentication
    let request = Request::builder()
        .uri("/admin/api/entities/adminonlydata/1")
        .body(Body::empty())
        .unwrap();
