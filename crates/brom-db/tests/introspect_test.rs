...
fn test_introspect_empty_db() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(tables.is_empty(), "Expected empty vector of tables, found: {:?}", tables);
...
fn test_introspect_single_table() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let id_col = posts.columns.iter().find(|c| c.name == "id").expect("id col");
    assert!(id_col.is_pk);
    assert_eq!(id_col.col_type, "TEXT");
    
    let title_col = posts.columns.iter().find(|c| c.name == "title").expect("title col");
    assert!(title_col.not_null);
    
    let views_col = posts.columns.iter().find(|c| c.name == "views").expect("views col");
...
fn test_introspect_foreign_keys() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 3. Assert
    let posts = tables.iter().find(|t| t.name == "posts").expect("posts table");
