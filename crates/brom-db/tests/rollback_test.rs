...
fn test_rollback_multi_step() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        let conn = pool.get()?;
        let t1_exists: i64 = conn.query_row("SELECT COUNT(*) FROM sqlite_master WHERE name='t1'", [], |r| r.get(0))?;
        let t2_exists: i64 = conn.query_row("SELECT COUNT(*) FROM sqlite_master WHERE name='t2'", [], |r| r.get(0))?;
